...
import ru.yandex.practicum.shop.service.CacheService;
...
public class ProductServiceImpl implements ProductService {
    ...
    public Mono<Page<ProductDTO>> getProductsByFilter(FilterProductDTO filter) {
        ...
        return itemService.getProductsFromCart()
                .zipWith(getSortedListProduct(filter))
                .map(tuple -> {
                    List<ProductDTO> productFromCart = tuple.getT1();
    ...
    public Mono<ProductDTO> getProductById(Integer productId) {
        return cacheService.getProductByIdFromCache(productId)
                ...
                .flatMap(product ->
                               itemService.findByOrderIdAndProductId(product.getId())
